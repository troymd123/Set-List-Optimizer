...
// ── PDF Export ────────────────────────────────────────────────────────────────
const exportDOCX = async (songs, analysis, vocalType, generatedSetlist) => {
  // Load docx library dynamically if not present
  if (!window.docx) {
  ...
  const bandName = state.activeBand || 'Setlist Optimizer';
  ...
  const txt = (text, opts={}) => new Paragraph({
    ...
    children:[new TextRun({text:String(text||''),bold:opts.bold,size:opts.size||22,color:opts.color||'FFFFFF',font:'Arial'})]
  });
  const songRow = (song, num) => new Paragraph({
    spacing:{before:40,after:40},
    children:[
      new TextRun({text:String(num).padStart(2,'0')+'  ',size:18,color:'888888',font:'Arial'}),
      new TextRun({text:song.title||'',bold:true,size:22,color:'FFFFFF',font:'Arial'}),
      song.artist?new TextRun({text:'   '+song.artist,size:18,color:'999999',font:'Arial'}):new TextRun(''),
      song.duration?new TextRun({text:'  ⏱ '+song.duration,size:16,color:'666666',font:'Arial'}):new TextRun(''),
      (song.tuning&&song.tuning!=='Standard')?new TextRun({text:'  ['+song.tuning+(song.capo>0?' Capo '+song.capo:'')+']',size:16,color:'CC5533',font:'Arial'}):new TextRun(''),
    ]
  });

  // Build content — multi-set or single
  const children = [
    txt(bandName, {bold:true,size:36,color:'E85D3A',after:40}),
    txt(date, {size:18,color:'666666',after:20}),
    txt(vocalType+' vocals · '+songs.length+' available songs', {size:18,color:'555555',after:40}),
    hr(),
  ];

  if (generatedSetlist?.sets?.length) {
    // Multi-set
    generatedSetlist.sets.forEach((set,si) => {
      children.push(txt('SET '+(si+1), {bold:true,size:28,color:'E85D3A',before:160,after:80}));
      const sl = set.setlist||[];
      sl.forEach((s,i) => children.push(songRow(s,i+1)));
      const setDur = sl.reduce((a,s)=>{
        if(!s.duration)return a;
        const [m,sec]=(s.duration||'0:00').split(':').map(Number);
        return a+m*60+(sec||0);
      },0);
      if(setDur) children.push(txt('Set runtime: '+formatTotal(setDur),{size:16,color:'555555',before:40,after:40}));
      if(si<generatedSetlist.sets.length-1)
        children.push(txt('— BREAK '+(state.breakMinutes||15)+' MINUTES —',{bold:true,size:20,color:'555555',center:true,before:120,after:120}));
    });
    if(generatedSetlist.notes) {
      children.push(hr());
      children.push(txt('NOTES',{bold:true,size:22,color:'E8A63A',before:80}));
      children.push(txt(generatedSetlist.notes,{size:18,color:'AAAAAA',after:80}));
    }
  } else {
    // Single flat setlist
    const list = generatedSetlist?.setlist || songs;
    children.push(txt('SETLIST',{bold:true,size:28,color:'FFFFFF',before:80,after:80}));
    list.forEach((s,i)=>children.push(songRow(s,i+1)));
  }

  if(analysis) {
    children.push(hr());
    children.push(txt('AI ANALYSIS — FLOW SCORE: '+(analysis.flowScore||'?')+'/10',{bold:true,size:24,color:'3AE8A6',before:80,after:60}));
    if(analysis.assessment) children.push(txt(analysis.assessment,{size:18,color:'AAAAAA',after:60}));
    if(analysis.energyArc){children.push(txt('ENERGY ARC',{bold:true,size:20,color:'E8A63A',before:60,after:40}));children.push(txt(analysis.energyArc,{size:18,color:'BBBBBB'}));}
    if((analysis.moves||[]).length){
      children.push(txt('SUGGESTED MOVES',{bold:true,size:20,color:'3A9EE8',before:80,after:40}));
      analysis.moves.forEach(m=>{
        children.push(txt('"'+m.song+'" — position #'+m.currentPos+' → #'+m.suggestedPos,{bold:true,size:19,color:'DDDDDD',before:40}));
        children.push(txt(m.reason,{size:16,color:'888888'}));
      });
    }
  }
  ...
  a.download = (bandName+' Setlist '+new Date().toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})).replace(/[^a-zA-Z0-9 ]/g,'-')+'.docx';
  a.click();
};
// Keep PDF alias pointing to DOCX for compatibility
const exportPDF = exportDOCX;
...
const h = (tag, attrs={}, ...children) => {
  ...
  for (const [k,v] of Object.entries(attrs)) {
    ...
    else if (k==='value') { valueAttr = v; if (tag==='input'||tag==='textarea') el.setAttribute('value', v); }
  ...
  // For <select>, value must be set as a property AFTER options are appended
  if (tag==='select' && valueAttr!==undefined) el.value = valueAttr;
  return el;
};
const render = () => {
  if (!rootEl) rootEl = document.getElementById('root');
...
const saveSetlist = (name, songs) => {
  ...
  const entry = {
    ...
    songs,
    sets: gs?.sets || null,           // preserve multi-set structure
    breakMinutes: state.breakMinutes,
    totalMinutes: gs?.totalEstimatedMinutes || null,
    notes: gs?.notes || '',
    createdAt: Date.now(),
  };
...
const analyze = async () => {
  ...
  try {
    ...
    // Build setlist description — if multi-set, label each set and mark breaks
    let setlistDescription;
    const gs = state.generatedSetlist;
    const viewedSl = state.viewingSetlist ? (state.savedSetlists||[]).find(s=>s.id===state.viewingSetlist) : null;
    const hasSets = gs?.sets?.length>1 || viewedSl?.sets?.length>1;
    if (hasSets) {
      const sets = gs?.sets || viewedSl?.sets || [];
      const breakMin = state.breakMinutes || viewedSl?.breakMinutes || 15;
      let pos = 0;
      setlistDescription = sets.map((set,si)=>{
        const songs = set.setlist||[];
        const lines = songs.map(s=>{
          pos++;
          return `${pos}. "${s.title}" by ${s.artist||'Unknown'} | Energy:${s.energy||'?'} | Tuning:${s.tuning||'Standard'}${s.capo>0?' Capo '+s.capo:''} | BPM:${s.bpm||'?'} | Key:${s.key||'?'} | Duration:${s.duration||'?'}`;
        }).join('\n');
        return '--- SET '+(si+1)+' ---\n'+lines+(si<sets.length-1?'\n--- BREAK: '+breakMin+' minutes ---':'');
      }).join('\n');
    } else {
      setlistDescription = target.songs.map((s,i)=>`${i+1}. "${s.title}" by ${s.artist||'Unknown'}${s.favorite?' [FAVORITE]':''} | Genre:${s.genre||'?'} | BPM:${s.bpm||'?'} | Key:${s.key||'?'} | Tuning:${s.tuning||'Standard'}${s.capo>0?' Capo '+s.capo:''} | Energy:${s.energy||'?'} | Vocal:${s.vocal||'?'} | Duration:${s.duration||'?'}`).join('\n');
    }
...
- Opener of each set: medium-high energy, recognizable, re-engages the crowd after any break.
- Energy moves in waves of 2-4 songs per arc — never a flat straight ramp. Real crowds need breathing room.
- Peak song (highest energy/most anthemic) lands roughly 2/3 through EACH SET, not at the very end.
- Closer of each set: strong, satisfying — the last impression before any break or end of show.
- No more than 2 ballads/low-energy songs per set, never back to back.
- Avoid two songs in near-identical key/tempo/feel back to back — vary it.
- CRITICAL: minimize tuning/capo changes between consecutive songs WITHIN A SET. Cluster same-tuning songs as contiguous blocks. Tuning changes cost 30-90 seconds of dead stage time.
${favTitles.length ? '- MUST-PLAY FAVORITES must stay in the set: '+favTitles.join(', ')+'.' : ''}
${hasSets ? '- IMPORTANT: This is a MULTI-SET show with breaks. Each set is its own performance arc. DO NOT give advice that treats this as one continuous run-through. The break resets audience energy — the opener of Set 2+ should re-engage the crowd like a fresh start. Analyze each set independently for its own opener/peak/closer arc.' : ''}

Analyzing: ${target.label}
${hasSets?'(Multi-set show — sets separated by breaks are shown below)':''}
${setlistDescription}
Vocal type filter: ${state.vocalType}

Give specific, performer-minded feedback — reference actual song titles and real on-stage reasons (dead air from tuning changes, energy pacing, crowd re-engagement after breaks, opener/closer fit per set). Keep all text fields under 100 words. Respond ONLY in JSON (no markdown):
{"assessment":"...","moves":[{"song":"...","currentPos":N,"suggestedPos":N,"reason":"..."},{"song":"...","currentPos":N,"suggestedPos":N,"reason":"..."},{"song":"...","currentPos":N,"suggestedPos":N,"reason":"..."}],"replacements":[{"song":"...","reason":"...","suggestion":"..."},{"song":"...","reason":"...","suggestion":"..."}],"additions":[{"title":"...","artist":"...","reason":"...","position":"..."},{"title":"...","artist":"...","reason":"...","position":"..."}],"energyArc":"...","keyFlowNotes":"...","flowScore":N,"flowScoreNote":"..."}`
    ,4000);
...
// Row for saved setlist expanded view
const buildSavedSongRow = (song, i) => {
  const gc = gcColor(song.genre||'Other');
  return h('div',{style:{display:'flex',alignItems:'center',gap:'8px',padding:'7px 4px',borderBottom:'1px solid rgba(255,255,255,0.04)'}},
    h('span',{style:{color:'#333',fontSize:'11px',fontFamily:'monospace',minWidth:'22px'}},String(i+1).padStart(2,'0')),
    h('div',{style:{flex:1,minWidth:0}},
      h('div',{style:{fontFamily:"'Bebas Neue',sans-serif",fontSize:'14px',letterSpacing:'1px',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},song.title),
      h('div',{style:{fontSize:'11px',color:'#555',display:'flex',gap:'6px',flexWrap:'wrap',marginTop:'2px'}},
        song.artist&&h('span',{},song.artist),
        song.duration&&h('span',{},'⏱ '+song.duration),
        (song.tuning&&song.tuning!=='Standard')&&h('span',{style:{color:'#e85d3a'}},song.tuning),
        song.capo>0&&h('span',{style:{color:'#e8a63a'}},'Capo '+song.capo),
        song.energy&&h('span',{},song.energy)
      )
    )
  );
};
...
const buildApp = () => {
  ...
  // Duration: only available songs matching vocal filter
  const vocalFilteredSongs = songs.filter(s => {
    if (s.unavailable) return false;
    if (vocalType==='Male') return !s.vocal || s.vocal==='Male' || s.vocal==='Duet' || s.vocal==='Group';
    if (vocalType==='Female') return !s.vocal || s.vocal==='Female' || s.vocal==='Duet' || s.vocal==='Group';
    return true;
  });
  const totalSec=totalDuration(vocalFilteredSongs);
  ...
  const addForm=h('div',{className:'card'},
    ...
    h('div',{style:{display:'flex',gap:'8px',marginTop:'10px',flexWrap:'wrap'}},
      ...
      songs.length>0&&h('button',{className:'btn btn-ghost',style:{marginLeft:'auto'},onClick:()=>exportDOCX(songs,analysis,vocalType,state.generatedSetlist)},'↓ Word')
    ),
  ...
  const buildSavedSetlistsTab = () => {
    ...
    return h('div',{},
      ...
      ...saved.map(sl=>{
        const isViewing = state.viewingSetlist===sl.id;
        const isExpanded = state.expandedSetlist===sl.id;
        return h('div',{style:{background:isViewing?'rgba(58,232,166,0.06)':'rgba(255,255,255,0.03)',border:isViewing?'1px solid rgba(58,232,166,0.3)':'1px solid rgba(255,255,255,0.07)',borderRadius:8,padding:'12px 14px',marginBottom:'8px'}},
          h('div',{style:{display:'flex',alignItems:'center',gap:'8px',marginBottom:'6px',flexWrap:'wrap'}},
            ...
            h('span',{style:{color:'#555',fontSize:'11px'}},sl.songs.length+' songs · '+(sl.totalMinutes?sl.totalMinutes+' min':'')),
            h('button',{className:'btn',style:{fontSize:'11px',padding:'4px 10px',
              background:isViewing?'rgba(58,232,166,0.25)':'rgba(58,158,232,0.12)',
              color:isViewing?'#3ae8a6':'#3a9ee8',border:'1px solid rgba(58,158,232,0.2)'},
              onClick:()=>setState({viewingSetlist:isViewing?null:sl.id,generatedSetlist:null})},
              isViewing?'Selected ✓':'Select for Optimize'),
            h('button',{className:'btn',style:{fontSize:'11px',padding:'4px 10px',background:'rgba(255,255,255,0.06)',color:'#bbb',border:'1px solid rgba(255,255,255,0.09)'},
              onClick:()=>setState({expandedSetlist:isExpanded?null:sl.id})},isExpanded?'▲ Hide':'▼ View'),
          ...
          isExpanded && h('div',{style:{marginTop:'10px'}},
            // If setlist has sets (multi-set), show with set headers; otherwise flat list
            (sl.sets||[]).length > 0
              ? sl.sets.map((set,si)=>h('div',{style:{marginBottom:'12px'}},
                  h('div',{style:{fontSize:'10px',fontWeight:'700',letterSpacing:'2px',color:'#e85d3a',marginBottom:'6px'}},'SET '+(si+1)),
                  ...(set.setlist||[]).map((song,i)=>buildSavedSongRow(song,i)),
                  si<sl.sets.length-1&&h('div',{style:{textAlign:'center',padding:'6px',color:'#444',fontSize:'10px',letterSpacing:'2px',borderTop:'1px solid rgba(255,255,255,0.05)',marginTop:'4px'}},
                    '— BREAK '+(sl.breakMinutes||15)+' MIN —')
                ))
              : sl.songs.map((song,i)=>buildSavedSongRow(song,i))
          ),
          !isExpanded && h('div',{style:{display:'flex',flexWrap:'wrap',gap:'4px'}},
  ...
  let tabContent;
  if(tab==='setlist') {
    if (!songs.length) tabContent=h('div',{className:'empty-state'},h('div',{className:'empty-icon'},'🎙️'),'No songs yet — add above or upload a CSV');
    else {
      const displaySongs = vocalType==='Mixed' ? songs : songs.filter(s=>{
        if(vocalType==='Male') return !s.vocal||s.vocal==='Male'||s.vocal==='Duet'||s.vocal==='Group';
        return !s.vocal||s.vocal==='Female'||s.vocal==='Duet'||s.vocal==='Group';
      });
      tabContent = h('div',{},
        vocalType!=='Mixed'&&h('div',{style:{fontSize:'11px',color:'#555',letterSpacing:'1px',marginBottom:'8px'}},
          `Showing ${displaySongs.length} of ${songs.length} songs for ${vocalType} vocal — filter does not affect the library`),
        ...displaySongs.map((s,i)=>buildSongCard(s,songs.indexOf(s)))
      );
    }
  }
