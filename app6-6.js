...
Setlist:
${target.songs.map((s,i)=>`${i+1}. "${s.title}" by ${s.artist||'Unknown'} | Genre:${s.genre||'?'} | BPM:${s.bpm||'?'} | Key:${s.key||'?'} | Tuning:${s.tuning||'Standard'}${s.capo>0?' Capo '+s.capo:''} | Energy:${s.energy||'?'} | Vocal:${s.vocal||'?'} | Duration:${s.duration||'?'}`).join('\n')}
