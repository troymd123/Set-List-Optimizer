...
const handleSpotifyCallback = async () => {
  ...
  try {
    ...
    // Exchange code via worker — server-side, no CORS restrictions
    let data;
    try {
      data = await exchangeCodeViaWorker(code, verifier, clientId, redirectUri);
    } catch(fetchErr) {
      throw new Error('Worker exchange failed: ' + fetchErr.message);
    }
...
const exchangeCodeViaWorker = async (code, verifier, clientId, redirectUri) => {
  const res = await fetch(SPOTIFY_PROXY + '/exchange', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Code': code,
  ...
  const data = await res.json();
  if (!res.ok || data.error)
    throw new Error('Exchange failed: ' + (data.error_description||data.error||res.status));
  return data;
};
...
const fetchSpotifyPlaylist = async (url) => {
  ...
  if (!res.ok || data.error) {
    if (res.status===401) {
      ...
      throw new Error('Token expired — paste URL and tap Import again to reconnect');
    }
...
const startSpotifyOAuth = (pendingUrl) => {
  ...
  // Stop the app from re-rendering and overwriting this screen
  rootEl = null;

  // Show the Spotify login screen - clicking the link is the guaranteed way in
  document.body.innerHTML = '';
  const loginDiv = document.createElement('div');
  loginDiv.style.cssText = 'position:fixed;inset:0;background:#090909;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:20px;padding:24px';
  loginDiv.innerHTML = '<div style="color:#1DB954;font-family:sans-serif;font-size:24px;font-weight:700;letter-spacing:2px">CONNECT TO SPOTIFY</div>'
    + '<div style="color:#777;font-size:13px;text-align:center;max-width:280px">Tap the button below to log in to Spotify. You only need to do this once.</div>'
    + '<a id="sp-login-btn" href="' + authUrl + '" style="display:block;background:#1DB954;color:#000;padding:16px 40px;border-radius:50px;font-weight:700;text-decoration:none;font-size:17px;text-align:center">Log in with Spotify</a>'
    + '<div style="color:#444;font-size:11px;text-align:center">After logging in you will return here automatically</div>';
  document.body.appendChild(loginDiv);
  
  // Also try auto-navigate after a short delay
    ...
    h('div',{className:'sp-import-row'},
      ...
      h('button',{
        ...
        onClick:loadingSpotify?null:()=>{
          ...
          // If no token, redirect immediately from this synchronous tap handler
