...
// PKCE is the correct flow for hosted browser apps — user logs in once,
// token cached locally and auto-refreshed.

const SP_TOKEN_STORE    = 'sp_access_token';
const SP_REFRESH_STORE  = 'sp_refresh_token';
const SP_EXPIRY_STORE   = 'sp_token_expiry';
const SP_VERIFIER_STORE = 'sp_code_verifier';
const SP_PENDING_URL    = 'sp_pending_url';

const generateRandomString = len => {
  const arr = new Uint8Array(Math.ceil(len * 3/4));
  crypto.getRandomValues(arr);
  return btoa(String.fromCharCode(...arr)).replace(/[+/=]/g,'').slice(0,len);
};
...
const handleSpotifyCallback = async (code, error) => {
  ...
  try {
    ...
    localStorage.setItem(SP_REFRESH_STORE, data.refresh_token||'');
    ...
    if (pendingUrl) {
      // Update state first, then import after render
      state.spotifyUrl = pendingUrl;
      setState({spotifyUrl:pendingUrl, spotifyStatus:{ok:true,msg:'Connected! Starting import...'}});
      setTimeout(() => importFromSpotify(pendingUrl), 600);
    } else {
      setState({spotifyStatus:{ok:true,msg:'Connected to Spotify — now paste a playlist URL and tap Import'}});
    }
  } catch(e) {
    setState({spotifyStatus:{ok:false,msg:'Auth failed: '+e.message}});
  }
  return true;
};
...
const refreshSpotifyToken = async () => {
  const clientId = localStorage.getItem(SP_CLIENT_STORE)||'';
  const refresh  = localStorage.getItem(SP_REFRESH_STORE)||'';
  ...
  try {
    ...
    localStorage.setItem(SP_TOKEN_STORE,  data.access_token);
    localStorage.setItem(SP_EXPIRY_STORE, String(Date.now()+(data.expires_in-60)*1000));
    if (data.refresh_token) localStorage.setItem(SP_REFRESH_STORE, data.refresh_token);
    return true;
  } catch(e) { return false; }
};

const getSpotifyToken = async (pendingUrl='') => {
  const clientId = localStorage.getItem(SP_CLIENT_STORE)||'';
  ...
  if (await refreshSpotifyToken()) return localStorage.getItem(SP_TOKEN_STORE);
  await startSpotifyLogin(pendingUrl);
  return null;
};
...
const isSpotifyConnected = () => {
  ...
  const expiry = parseInt(localStorage.getItem(SP_EXPIRY_STORE)||'0');
  const refresh= localStorage.getItem(SP_REFRESH_STORE)||'';
  return !!(token && (Date.now() < expiry || refresh));
};

const disconnectSpotify = () => {
  [SP_TOKEN_STORE,SP_REFRESH_STORE,SP_EXPIRY_STORE,SP_VERIFIER_STORE,SP_PENDING_URL].forEach(k=>localStorage.removeItem(k));
...
const fetchSpotifyPlaylist = async (url) => {
  ...
  const token = await getSpotifyToken(url);
  if (!token) return null;
  const headers = { Authorization:'Bearer '+token };
  const res = await fetch(SPOTIFY_API+'/playlists/'+id,{headers});
  if (res.status===401) {
    [SP_TOKEN_STORE,SP_REFRESH_STORE,SP_EXPIRY_STORE].forEach(k=>localStorage.removeItem(k));
  ...
  const data = await res.json();
  // Surface any Spotify error messages directly
  if (data.error) throw new Error('Spotify: ' + (data.error.message||JSON.stringify(data.error)));

  // Dump first 500 chars of raw response into error so we can see structure
  const rawDump = JSON.stringify(data).slice(0,500);

  // data.items is present — inspect its first element directly
  const rawItems = data.items || (data.tracks && data.tracks.items) || [];
  if (!rawItems.length) throw new Error('Empty items array. Raw: ' + rawDump);

  const firstItem = rawItems[0];
  const firstItemKeys = firstItem ? Object.keys(firstItem).join(',') : 'null';
  
  // Items may be track objects directly, or {track:{...}} wrappers, or {added_at, track:{...}}
  let songs = [];
  for (const item of rawItems) {
    if (!item) continue;
    // Case 1: item is {added_at, track: {name,...}}
    if (item.track && item.track.name) {
      const t = item.track;
      const sec = Math.round((t.duration_ms||0)/1000);
      songs.push({ id:mkId(), title:t.name, artist:(t.artists||[]).map(a=>a.name).join(', '),
        genre:'Other', bpm:'', key:'', energy:'Medium', spotifyId:t.id||'',
        duration: sec>0 ? Math.floor(sec/60)+':'+String(sec%60).padStart(2,'0') : '' });
    }
    // Case 2: item is the track directly {name, artists,...}
    else if (item.name && item.artists) {
      const sec = Math.round((item.duration_ms||0)/1000);
      songs.push({ id:mkId(), title:item.name, artist:(item.artists||[]).map(a=>a.name).join(', '),
        genre:'Other', bpm:'', key:'', energy:'Medium', spotifyId:item.id||'',
        duration: sec>0 ? Math.floor(sec/60)+':'+String(sec%60).padStart(2,'0') : '' });
    }
  }

  // Paginate if there are more pages
  let nextUrl = (data.tracks && data.tracks.next) || data.next || null;
  while (nextUrl) {
    const pg = await fetch(nextUrl, {headers});
    const pd = await pg.json();
    const pageItems = pd.items || [];
    for (const item of pageItems) {
      if (!item) continue;
      if (item.track && item.track.name) {
        const t = item.track, sec = Math.round((t.duration_ms||0)/1000);
        songs.push({ id:mkId(), title:t.name, artist:(t.artists||[]).map(a=>a.name).join(', '),
          genre:'Other', bpm:'', key:'', energy:'Medium', spotifyId:t.id||'',
          duration: sec>0 ? Math.floor(sec/60)+':'+String(sec%60).padStart(2,'0') : '' });
      } else if (item.name && item.artists) {
  ...
  if (!songs.length) {
    throw new Error('Parsed 0 songs. Items:' + rawItems.length + ' firstItemKeys:' + firstItemKeys + ' sample:' + JSON.stringify(firstItem).slice(0,300));
  }
...
const importFromSpotify = async (overrideUrl) => { overrideUrl = (typeof overrideUrl === 'string') ? overrideUrl : '';
  ...
  setState({loadingSpotify:true, spotifyStatus:{ok:null, msg:'Connecting to Spotify...'}});
  try {
    const result = await fetchSpotifyPlaylist(url);
    // returns null when redirecting to Spotify login page — that is expected on first use
    if (!result) {
      setState({loadingSpotify:false, spotifyStatus:{ok:null, msg:'Redirecting to Spotify login — you will return here automatically...'}});
      return;
    }
...
// Handle Spotify OAuth callback (?code= or ?error= in URL)
(async () => {
  const params = new URLSearchParams(window.location.search);
  const code = params.get('code'), error = params.get('error');
  if (code || error) {
    window.history.replaceState({}, '', window.location.pathname);
    setState({spotifyStatus:{ok:null, msg:'Completing Spotify login…'}});
    await handleSpotifyCallback(code, error);
  }
})();
