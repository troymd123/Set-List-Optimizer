...
const SP_TOKEN_VERSION  = 'sp_token_v2'; // bump this to force re-auth on scope change
...
const isSpotifyConnected = () => {
  // Wipe any token stored before v2 scope (playlist-read-private was added)
  if (!localStorage.getItem(SP_TOKEN_VERSION)) {
    [SP_TOKEN_STORE,SP_REFRESH_STORE,SP_EXPIRY_STORE].forEach(k=>localStorage.removeItem(k));
    localStorage.setItem(SP_TOKEN_VERSION,'1');
    return false;
  }
  const token  = localStorage.getItem(SP_TOKEN_STORE)||'';
  const expiry = parseInt(localStorage.getItem(SP_EXPIRY_STORE)||'0');
...
const disconnectSpotify = () => {
  [SP_TOKEN_STORE,SP_REFRESH_STORE,SP_EXPIRY_STORE,SP_VERIFIER_STORE,SP_PENDING_URL,SP_TOKEN_VERSION].forEach(k=>localStorage.removeItem(k));
