...
// Implicit Grant flow — token returned in URL hash, no server exchange needed.
// Simpler and more reliable for static hosted PWAs.
...
const startSpotifyLogin = (pendingUrl='') => {
...
// Called on page load — reads #access_token=...&expires_in=... from URL hash
const handleSpotifyCallback = () => {
  ...
  if (error) { setState({spotifyStatus:{ok:false,msg:'Spotify login cancelled'}}); return true; }
  if (!token) return false;
  localStorage.setItem(SP_TOKEN_STORE,  token);
  localStorage.setItem(SP_EXPIRY_STORE, String(Date.now()+(expiresIn-60)*1000));
  const pendingUrl = localStorage.getItem(SP_PENDING_URL)||'';
  localStorage.removeItem(SP_PENDING_URL);
  if (pendingUrl) {
    state.spotifyUrl = pendingUrl;
    setState({spotifyUrl:pendingUrl, spotifyStatus:{ok:true,msg:'Connected! Importing playlist...'}});
    setTimeout(() => importFromSpotify(pendingUrl), 400);
  } else {
    setState({spotifyStatus:{ok:true,msg:'Connected to Spotify — paste a playlist URL and tap Import'}});
  }
  return true;
};

const getSpotifyToken = (pendingUrl='') => {
  const clientId = localStorage.getItem(SP_CLIENT_STORE)||'';
...
const disconnectSpotify = () => {
  [SP_TOKEN_STORE, SP_EXPIRY_STORE, SP_PENDING_URL].forEach(k=>localStorage.removeItem(k));
...
const fetchSpotifyPlaylist = async (url) => {
  ...
  const token = getSpotifyToken(url);
...
// Handle Spotify implicit grant callback (token in URL #hash)
