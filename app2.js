...
const startSpotifyLogin = async (pendingUrl='') => {
  ...
  localStorage.setItem(SP_STATE_STORE,    state);
...
const handleSpotifyCallback = async () => {
  ...
  // Clear query string immediately so refresh doesn't re-trigger
  ...
  if (savedState && retState !== savedState) {
  ...
  try {
    ...
    setState({spotifyStatus:{ok:false,msg:'Login failed: '+e.message}});
  }
  return true;
};
...
(async () => { await handleSpotifyCallback(); })();
