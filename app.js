...
const fetchSpotifyPlaylist = async (url) => {
  ...
  // Try every possible location tracks could be
  let items = [];
  let nextUrl = null;

  if (data.tracks && Array.isArray(data.tracks.items)) {
    // Standard playlist response: data.tracks.items
    items = data.tracks.items;
    nextUrl = data.tracks.next || null;
  } else if (Array.isArray(data.items)) {
    // Flattened response: data.items
    items = data.items;
    nextUrl = data.next || null;
  } else if (data.tracks && typeof data.tracks === 'object') {
    // data.tracks exists but no .items — show its keys for debugging
    throw new Error('data.tracks exists but has no items. tracks keys: ' + Object.keys(data.tracks).join(', '));
  } else {
    throw new Error('Could not find tracks. Keys: ' + Object.keys(data).join(', ') + ' | items type: ' + (typeof data.items) + ' | sample: ' + JSON.stringify(data.items ? data.items[0] : null).slice(0,200));
  }
  ...
  // Each item should be {track: {name, artists, duration_ms}}
  // Filter nulls (local files, podcasts come back as null tracks)
  const validItems = items.filter(i => i && i.track && i.track.name && i.track.artists);

  if (!validItems.length) {
    const sample = JSON.stringify(items[0]).slice(0, 300);
    throw new Error('items found (' + items.length + ') but none have valid tracks. First item: ' + sample);
  }

  const songs = validItems.map(i => {
    const t = i.track;
