...
const handleSpotifyCallback = async (code, error) => {
  ...
  try {
    ...
    if (pendingUrl) {
      setState({spotifyUrl:pendingUrl, spotifyStatus:{ok:true,msg:'Connected to Spotify! Importing...'}});
      setTimeout(() => importFromSpotify(pendingUrl), 400);
    } else {
      setState({spotifyStatus:{ok:true,msg:'Connected to Spotify'}});
    }
  } catch(e) {
...
const fetchSpotifyPlaylist = async (url) => {
  ...
  const res = await fetch(SPOTIFY_API+'/playlists/'+id+'?fields=name,tracks(items(track(name,artists,duration_ms)),next,total)',{headers});
  if (res.status===401) {
  ...
  const data = await res.json();
  let items = data.tracks.items||[], nextUrl = data.tracks.next;
  while (nextUrl) {
    const pg = await fetch(nextUrl,{headers}), pd = await pg.json();
    items = items.concat(pd.items||[]); nextUrl = pd.next;
  }
  const songs = items.filter(i=>i.track?.name).map(i => {
