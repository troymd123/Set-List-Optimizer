...
const getSpotifyToken = async () => {
  ...
  if (!clientId||!clientSecret) throw new Error('NO_SP_CREDS');
  const res = await fetch(SPOTIFY_TOKEN,{
    method:'POST',
    headers:{'Content-Type':'application/x-www-form-urlencoded'},
    body: `grant_type=client_credentials&client_id=${encodeURIComponent(clientId)}&client_secret=${encodeURIComponent(clientSecret)}`
  });
  if (!res.ok) throw new Error('SP_AUTH_FAILED');
  const data = await res.json();
...
const fetchSpotifyPlaylist = async (url) => {
  ...
  // First fetch — get playlist name + first page
  const res = await fetch(`${SPOTIFY_API}/playlists/${id}?fields=name,tracks(items(track(name,artists,duration_ms,album(name))),next,total)`,{headers});
  if (!res.ok) throw new Error('SP_FETCH_FAILED');
  const data = await res.json();
  ...
  while (nextUrl) {
    const page = await fetch(nextUrl,{headers});
  ...
  const songs = items
    ...
    .map(i => {
      ...
      return {
        ...
        bpm: '',
        key: '',
        duration: durationSec>0 ? `${m}:${String(s).padStart(2,'0')}` : '',
        energy: 'Medium',
        spotifyId: t.id||'',
      };
    });
...
const importFromSpotify = async () => {
  const url = state.spotifyUrl.trim();
  ...
  if (!localStorage.getItem(SP_CLIENT_STORE)||!localStorage.getItem(SP_SECRET_STORE)) {
    setState({spotifyStatus:{ok:false,msg:'Add your Spotify Client ID & Secret in Settings first'}}); return;
  }
  setState({loadingSpotify:true, spotifyStatus:null});
  try {
    const {name, songs:imported} = await fetchSpotifyPlaylist(url);
    ...
    // Detect info for first 20 (avoid hammering API)
    imported.slice(0,20).forEach(s => detectSongInfo(s));
  } catch(err) {
    let msg = 'Import failed — check your Spotify credentials and playlist URL';
    if (err.message==='NO_SP_CREDS') msg='Add your Spotify Client ID & Secret in Settings';
    if (err.message==='SP_AUTH_FAILED') msg='Spotify auth failed — check your Client ID & Secret';
    if (err.message==='INVALID_URL') msg='Could not parse playlist ID from that URL';
...
const buildSettingsModal = () => {
  const {apiKey,spClientId,spClientSecret}=state;
    ...
    h('div',{className:'settings-box'},
      ...
      h('div',{className:'settings-section'},
        ...
        h('p',{className:'field-hint',style:{marginBottom:'12px'}},
          ...
          ' → Create an app → copy your Client ID & Secret.',h('br',{}),'',
          '2. In your Spotify app settings, set Redirect URI to: ',h('br',{}),'',
          h('code',{style:{color:'#1DB954',fontSize:'11px',wordBreak:'break-all'}}, window.location.origin+window.location.pathname)
        ),
        h('div',{className:'field-label'},'CLIENT ID'),
        h('input',{className:'inp',style:{marginBottom:'8px'},placeholder:'Paste Client ID',value:spClientId,onInput:e=>setState({spClientId:e.target.value})}),
        h('div',{className:'field-label'},'CLIENT SECRET'),
        h('div',{className:'api-inp-wrap'},
          h('input',{className:'inp',type:'password',placeholder:'Paste Client Secret',value:spClientSecret,onInput:e=>setState({spClientSecret:e.target.value})}),
          h('button',{className:'btn btn-green',onClick:()=>{
            localStorage.setItem(SP_CLIENT_STORE,state.spClientId);
            localStorage.setItem(SP_SECRET_STORE,state.spClientSecret);
...
const buildApp = () => {
  ...
  const hasSpotify=!!localStorage.getItem(SP_CLIENT_STORE)&&!!localStorage.getItem(SP_SECRET_STORE);
  ...
  const spotifyBox=h('div',{className:'sp-import-box'},
    h('div',{className:'sp-import-title'},
      '♫ Import from Spotify',
      !hasSpotify&&h('span',{style:{fontSize:'10px',color:'#555',fontWeight:'normal',letterSpacing:'0'}},' — add credentials in ⚙ Settings')
    ),
