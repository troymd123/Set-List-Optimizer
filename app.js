...
const fetchSpotifyPlaylist = async (url) => {
  ...
  // Tracks can be at data.tracks.items OR data.items depending on API response shape
  const tracksObj = data.tracks || data;
  ...
  if (!items.length) throw new Error('Playlist returned 0 tracks. Top-level keys: ' + Object.keys(data).join(', '));
  const songs = items
