...
const startSpotifyOAuth = (pendingUrl) => {
  ...
  // Show redirect indicator on screen
  document.body.innerHTML = '<div style="position:fixed;inset:0;background:#090909;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:16px"><div style="color:#1DB954;font-family:Bebas Neue,sans-serif;font-size:28px;letter-spacing:3px">OPENING SPOTIFY</div><div style="color:#555;font-size:13px">If nothing happens, tap the button below</div><a href="' + 'https://accounts.spotify.com/authorize?' + params + '" style="background:#1DB954;color:#000;padding:12px 28px;border-radius:8px;font-weight:700;text-decoration:none;font-size:15px">Open Spotify Login</a></div>';
  
  // Clear pre-computed values so they get regenerated
  ...
  // Try location first, fall back to open
  try { window.location.assign(authUrl); } catch(e) { window.open(authUrl,'_self'); }
};
