...
const handleSpotifyCallback = async () => {
  ...
  // Phase 2: Clean reload with saved code — only run if sp_ready flag is set
  ...
  sessionStorage.removeItem('sp_ready');
  ...
  try {
    ...
    // Show status directly on page - works even before app renders
    const showStatus = msg => {
      let sd = document.getElementById('sp-status-div');
      if (!sd) {
        sd = document.createElement('div');
        sd.id = 'sp-status-div';
        sd.style.cssText = 'position:fixed;top:0;left:0;right:0;background:#090909;color:#3ae8a6;font-family:monospace;font-size:12px;padding:16px;z-index:9999;white-space:pre-wrap';
        document.body.appendChild(sd);
      }
      sd.textContent += msg + '\n';
    };
    showStatus('Phase 2: exchanging token...');
    showStatus('worker: ' + tokenUrl);
    let res, data;
    try {
      ...
      showStatus('FETCH ERROR: ' + fetchErr.message);
      throw new Error('Fetch failed: ' + fetchErr.message);
    }
    let raw = '';
    try {
      raw = await res.text();
      showStatus('response: ' + raw.slice(0,120));
    } catch(readErr) {
      showStatus('READ ERROR: ' + readErr.message);
      throw new Error('Read failed: ' + readErr.message);
    }
    try {
      data = JSON.parse(raw);
    } catch(parseErr) {
      showStatus('PARSE ERROR. raw: ' + raw.slice(0,80));
      throw new Error('Parse failed: ' + raw.slice(0,80));
    }
    if (!res.ok || data.error) {
      throw new Error(data.error_description || data.error || 'Token exchange failed ' + res.status);
    }
    showStatus('token received! saving...');
    ...
    showStatus('saved! rendering app...');
    const sd = document.getElementById('sp-status-div');
    if (sd) setTimeout(()=>sd.remove(), 2000);
    ...
    const sd2 = document.getElementById('sp-status-div');
    if (sd2) { sd2.style.color='#e85d3a'; sd2.textContent += 'ERROR: ' + e.message; }
    else { const d=document.createElement('div'); d.style.cssText='position:fixed;top:0;left:0;right:0;background:#090909;color:#e85d3a;font-family:monospace;font-size:12px;padding:16px;z-index:9999'; d.textContent='Spotify error: '+e.message; document.body.appendChild(d); }
  }
  return true;
};
...
// Get token via Client Credentials — no user login needed for public playlists
const getSpotifyClientToken = async () => {
  const cached = sessionStorage.getItem('sp_cc_token');
  const expiry = parseInt(sessionStorage.getItem('sp_cc_expiry')||'0');
  if (cached && Date.now() < expiry) return cached;
  const clientId = localStorage.getItem(SP_CLIENT_STORE)||'';
  const clientSecret = localStorage.getItem(SP_SECRET_STORE)||'';
  if (!clientId || !clientSecret) throw new Error('Add your Spotify Client ID and Secret in ⚙ Settings');
  const res = await fetch(SPOTIFY_PROXY + '/cc-token', {
    method: 'POST',
  ...
  const data = await res.json();
  if (data.error) throw new Error('Spotify auth error: ' + (data.error_description||data.error));
  sessionStorage.setItem('sp_cc_token', data.access_token);
  sessionStorage.setItem('sp_cc_expiry', String(Date.now()+(data.expires_in-60)*1000));
  return data.access_token;
};

// Proxy a Spotify API call through the worker (avoids CORS)
const spotifyFetch = async (spotifyUrl, token) => {
  const res = await fetch(SPOTIFY_PROXY + '/proxy', {
    method: 'POST',
    headers: {'Content-Type':'application/json'},
    body: JSON.stringify({spotifyUrl, token}),
  });
...
const fetchSpotifyPlaylist = async (url) => {
  ...
  // Get playlist name via proxy
  const meta = await spotifyFetch(SPOTIFY_API+'/playlists/'+id+'?fields=name', token);
  if (meta.error) {
    const msg = meta.error.message||JSON.stringify(meta.error);
    if (meta.error.status===403||meta.error.status===401) 
      throw new Error('Access denied — make sure the playlist is set to Public in Spotify, and your Client ID & Secret are correct in Settings');
    throw new Error('Spotify: ' + msg);
  }
  const playlistName = meta.name || 'Spotify Playlist';
  
  // Fetch tracks via proxy
  let songs = [], tracksUrl = SPOTIFY_API+'/playlists/'+id+'/tracks?limit=100';
  while (tracksUrl) {
    const td = await spotifyFetch(tracksUrl, token);
    if (td.error) throw new Error('Tracks error: '+(td.error.message||td.error.status));
    for (const item of (td.items||[])) {
      ...
      songs.push({
        ...
        tuning:'Standard', capo:0, vocal:'', unavailable:false,
      });
    }
    tracksUrl = td.next||null;
  }
  if (!songs.length) throw new Error('No tracks found — make sure the playlist is Public in Spotify');
...
const importFromSpotify = async (overrideUrl) => { overrideUrl = (typeof overrideUrl === 'string') ? overrideUrl : '';
  ...
  if (!localStorage.getItem(SP_CLIENT_STORE)||!localStorage.getItem(SP_SECRET_STORE)) {
    setState({spotifyStatus:{ok:false,msg:'Add your Spotify Client ID and Secret in ⚙ Settings first'}});
    return;
  }
...
const buildSettingsModal = () => {
  ...
  return h('div',{className:'settings-modal',onClick:e=>e.target===e.currentTarget&&setState({showSettings:false})},
    h('div',{className:'settings-box'},
      ...
      // Spotify — Client Credentials (no login redirect needed for public playlists)
      h('div',{className:'settings-section'},
        h('div',{className:'settings-section-title',style:{color:'#1DB954'}},'♫ Spotify'),
        h('p',{className:'field-hint',style:{marginBottom:'12px'}},
          'Go to ',h('a',{href:'https://developer.spotify.com/dashboard',target:'_blank',style:{color:'#1DB954'}},'developer.spotify.com/dashboard'),
          ' → create an app → copy your Client ID and Client Secret below. No login redirect needed!'
        ),
        h('div',{className:'field-label'},'CLIENT ID'),
        h('input',{className:'inp',style:{marginBottom:'8px'},placeholder:'Paste Client ID',
          value:state.spClientId||'',
          onInput:e=>{state.spClientId=e.target.value;}}),
        h('div',{className:'field-label'},'CLIENT SECRET'),
        h('div',{className:'api-inp-wrap'},
          h('input',{className:'inp',type:'password',placeholder:'Paste Client Secret',
            value:state.spClientSecret||'',
            onInput:e=>{state.spClientSecret=e.target.value;}}),
          h('button',{className:'btn btn-green',onClick:()=>{
            const id = (state.spClientId||'').trim();
            const secret = (state.spClientSecret||'').trim();
            if (!id||!secret) return;
            localStorage.setItem(SP_CLIENT_STORE, id);
            localStorage.setItem(SP_SECRET_STORE, secret);
            sessionStorage.removeItem('sp_cc_token');
            sessionStorage.removeItem('sp_cc_expiry');
            setState({showSettings:false,spotifyStatus:{ok:true,msg:'Credentials saved! Paste a playlist URL and tap Import.'}});
          }},'Save')
        ),
        h('p',{className:'field-hint',style:{marginTop:'8px',color:'#3a3a3a'}},'Your playlist must be set to Public in Spotify.')
      ),
...
// INIT
// ─────────────────────────────────────────────────────────────────────────────
// Phase 1: arrived from Spotify with ?code= — save it and reload immediately for clean exchange
(function() {
  const sp = new URLSearchParams(window.location.search);
  const code = sp.get('code');
  if (code) {
    // Save code then reload so the fetch happens on a clean page load
    sessionStorage.setItem('sp_code', code);
    sessionStorage.setItem('sp_state', sp.get('state')||'');
    sessionStorage.setItem('sp_ready', '1');
    // Replace URL before reload so we don't loop
    // Navigate to clean URL - more reliable than reload() on Chrome Android
