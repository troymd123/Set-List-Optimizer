...
const handleSpotifyCallback = async () => {
  ...
  try {
    ...
    // Show error on screen for 8 seconds so user can read it
    const errDiv = document.createElement('div');
    errDiv.style.cssText = 'position:fixed;inset:0;background:#090909;color:#e85d3a;font-family:monospace;font-size:13px;padding:24px;z-index:9999;white-space:pre-wrap;overflow:auto';
    errDiv.textContent = 'Spotify login error:\n\n' + e.message + '\n\nThe app will reload in 8 seconds...';
    document.body.appendChild(errDiv);
    setTimeout(() => {
      errDiv.remove();
      if (!rootEl) rootEl = document.getElementById('root');
