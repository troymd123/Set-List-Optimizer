...
const handleSpotifyCallback = async () => {
  ...
  try {
    ...
    // Hide debug screen and render the app
    const dbg = document.getElementById('error-display');
    if (dbg) dbg.style.display = 'none';
    // Never auto-import — just show connected and restore the URL so user taps Import
    setState({
      spotifyUrl: pendingUrl||'',
      spotifyStatus:{ok:true, msg:'Connected to Spotify! ' + (pendingUrl ? 'Tap Import to load your playlist.' : 'Paste a playlist URL and tap Import.')}
    });
  } catch(e) {
    const dbg3 = document.getElementById('error-display');
    if (dbg3) dbg3.style.display = 'none';
