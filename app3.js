...
const handleSpotifyCallback = async () => {
  // Read from sessionStorage where we stashed it, or re-read URL if not cleared yet
  const params  = new URLSearchParams(window.location.search);
  const code    = sessionStorage.getItem('sp_code') || params.get('code');
  const error   = params.get('error');
  const retState= sessionStorage.getItem('sp_state') || params.get('state');
  sessionStorage.removeItem('sp_code');
  sessionStorage.removeItem('sp_state');
  if (!code && !error) return false;
  window.history.replaceState({}, '', window.location.pathname);
  if (!localStorage.getItem(SP_VERIFIER_STORE) && !error) return false;
...
const state = {
  ...
  editingId: null,
  editData: {},
  newSong: {title:'',artist:'',genre:'Pop',bpm:'',key:'',duration:'',energy:'Medium',vocal:''},
  targetMinutes: 60,
  numberOfSets: 1,
  breakMinutes: 15,
  showSettings: false,
...
const addSong = () => {
  ...
  saveToStorage(songs);
  setState({songs, newSong:{title:'',artist:'',genre:'Pop',bpm:'',key:'',duration:'',energy:'Medium',vocal:''}});
  detectSongInfo(song);
};
...
{"bpm":integer_or_null,"key":"string_or_null","energy":"Low|Medium|High","duration":"m:ss_or_null","genre":"Rock|Pop|R&B|Country|Jazz|Folk|Electronic|Hip-Hop|Soul|Blues|Other","vocal":"Male|Female|Duet|Group|"}`, 300
    );
    const updated = {
      ...song,
...
// Apply a generated setlist — replace current songs with it
const applyGeneratedSetlist = () => {
  if (!state.generatedSetlist?.setlist?.length) return;
  const newSongs = state.generatedSetlist.setlist.map(s => ({
    ...s, id:mkId(),
  ...
  setState({songs:newSongs, generatedSetlist:null, tab:'setlist'});
};
...
Setlist:
${state.songs.map((s,i)=>`${i+1}. "${s.title}" by ${s.artist||'Unknown'} | Genre:${s.genre} | BPM:${s.bpm||'?'} | Key:${s.key||'?'} | Energy:${s.energy} | Vocal:${s.vocal||'?'} | Duration:${s.duration||'?'}`).join('\n')}
...
const generateSetlist = async () => {
  ...
  try {
    ...
    const prompt = isMultiSet
      ? `You are a professional music director. Build ${state.numberOfSets} sets of ${state.targetMinutes} minutes each with ${state.breakMinutes}-minute breaks between sets. Vocals: ${state.vocalType}.
Available songs:
${state.songs.map((s,i)=>`${i+1}. "${s.title}" by ${s.artist||'Unknown'} | Genre:${s.genre} | BPM:${s.bpm||'?'} | Key:${s.key||'?'} | Energy:${s.energy} | Vocal:${s.vocal||'?'} | Duration:${s.duration||'?'}`).join('\n')}
Each set ~${Math.round(state.targetMinutes/3.5)} songs. Order each set with good energy arc. Keep reasons under 30 words.
Respond ONLY in JSON (no markdown):
{"sets":[{"setlist":[{"title":"...","artist":"...","genre":"...","bpm":"...","key":"...","duration":"...","energy":"..."}]}],"totalEstimatedMinutes":N,"notes":"...","swaps":[{"remove":"...","add":"...","artist":"...","reason":"..."}]}`
      : `You are a professional music director. Build the optimal ${state.targetMinutes}-minute setlist with ${state.vocalType} vocals.
Available songs:
${state.songs.map((s,i)=>`${i+1}. "${s.title}" by ${s.artist||'Unknown'} | Genre:${s.genre} | BPM:${s.bpm||'?'} | Key:${s.key||'?'} | Energy:${s.energy} | Vocal:${s.vocal||'?'} | Duration:${s.duration||'?'}`).join('\n')}
~${Math.round(state.targetMinutes/3.5)} songs. Keep reasons under 30 words.
Respond ONLY in JSON (no markdown):
{"setlist":[{"title":"...","artist":"...","genre":"...","bpm":"...","key":"...","duration":"...","energy":"..."}],"totalEstimatedMinutes":N,"notes":"...","swaps":[{"remove":"...","add":"...","artist":"...","reason":"..."}]}`;
...
const buildGenerateTab = () => {
  ...
  return h('div',{},
    h('div',{className:'generate-panel'},
      ...
      h('div',{className:'generate-controls',style:{flexWrap:'wrap',gap:'8px'}},
        h('span',{style:{fontSize:'12px',color:'#888'}},'Sets:'),
        h('input',{className:'inp',type:'number',min:'1',max:'5',value:numberOfSets,style:{maxWidth:'55px'},
          onInput:e=>setState({numberOfSets:parseInt(e.target.value)||1})}),
        h('span',{style:{fontSize:'12px',color:'#555'}},'×'),
        h('input',{className:'inp',type:'number',min:'15',max:'180',value:targetMinutes,style:{maxWidth:'70px'},
          onInput:e=>setState({targetMinutes:parseInt(e.target.value)||60})}),
        h('span',{style:{fontSize:'12px',color:'#555'}},'min each'),
        numberOfSets>1&&h('span',{style:{fontSize:'12px',color:'#888'}},'Break:'),
        numberOfSets>1&&h('input',{className:'inp',type:'number',min:'5',max:'60',value:breakMinutes,style:{maxWidth:'55px'},
          onInput:e=>setState({breakMinutes:parseInt(e.target.value)||15})}),
        numberOfSets>1&&h('span',{style:{fontSize:'12px',color:'#555'}},'min')
      ),
    ...
    generatedSetlist&&h('div',{style:{marginTop:'14px'}},
      h('div',{style:{display:'flex',alignItems:'center',gap:'10px',marginBottom:'12px',flexWrap:'wrap'}},
        ...
        h('button',{className:'btn btn-primary',style:{fontSize:'12px',padding:'6px 14px',marginLeft:'auto'},
          onClick:applyGeneratedSetlist},'✓ Apply to My Setlist')
      ),
  ...
  const addForm=h('div',{className:'card'},
    ...
    h('div',{className:'add-form'},
      h('input',{className:'inp',placeholder:'Song title *',value:newSong.title,
        onInput:e=>{ state.newSong={...state.newSong,title:e.target.value}; },
        onKeydown:e=>e.key==='Enter'&&addSong()}),
      h('input',{className:'inp inp-md',placeholder:'Artist',value:newSong.artist,
        onInput:e=>{ state.newSong={...state.newSong,artist:e.target.value}; }}),
...
// INIT
// ─────────────────────────────────────────────────────────────────────────────
// Stash Spotify callback params in sessionStorage immediately on load (codes expire fast)
(function() {
  const sp = new URLSearchParams(window.location.search);
  if (sp.get('code')) { sessionStorage.setItem('sp_code', sp.get('code')); sessionStorage.setItem('sp_state', sp.get('state')||''); }
})();
