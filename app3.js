...
const handleSpotifyCallback = async () => {
  ...
  try {
    ...
    dbgLog('Saved! Rendering app in 1s...');
    setTimeout(() => {
      try {
        if(dbg) dbg.style.display='none';
        // rootEl may not be set yet if callback fired before init — get it fresh
        ...
        if(dbg) { dbg.style.display='block'; dbg.style.color='#e85d3a'; dbg.textContent += ' RENDER ERROR: ' + renderErr.message + ' ' + (renderErr.stack||'').slice(0,200); }
      }
    }, 1000);
  } catch(e) {
