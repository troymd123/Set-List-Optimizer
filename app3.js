...
const addSong = () => {
  ...
  saveToStorage(songs);
  setState({songs, newSong:{title:'',artist:'',genre:'Pop',bpm:'',key:'',duration:'',energy:'Medium',vocal:'',tuning:'Standard',capo:0}});
  detectSongInfo(song);
};
const removeSong = id => {
    ...
    return h('div',{className:'song-card',style:{borderLeft:`3px solid ${gc}`,flexWrap:'wrap'}},
      h('div',{className:'edit-row'},
        ...
        h('input',{className:'inp inp-sm',placeholder:'Capo',type:'number',min:'0',max:'12',value:state.editData.capo||0,
          onInput:e=>setState({editData:{...state.editData,capo:parseInt(e.target.value)||0}})}),
...
const buildApp = () => {
  ...
  const addForm=h('div',{className:'card'},
    ...
    h('div',{className:'add-form'},
      ...
      h('input',{className:'inp inp-sm',placeholder:'Capo',type:'number',min:'0',max:'12',value:newSong.capo||0,
        onInput:e=>setState({newSong:{...state.newSong,capo:parseInt(e.target.value)||0}})})
    ),
