...
const handleSpotifyCallback = async () => {
  ...
  try {
    ...
    // First test worker reachability with a GET (should return 405)
    try {
      const pingRes = await fetch(tokenUrl, {method:'GET'});
      dbgLog('Worker ping status: ' + pingRes.status + ' (405=OK, other=problem)');
    } catch(pingErr) {
      dbgLog('Worker UNREACHABLE: ' + pingErr.message);
      throw new Error('Worker unreachable: ' + pingErr.message);
    }
