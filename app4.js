...
// Get anonymous Spotify token via their open token endpoint (no credentials needed)
const getSpotifyClientToken = async () => {
  ...
  // Spotify's open token endpoint — used by their web player, no auth needed
  const res = await fetch('https://open.spotify.com/get_access_token?reason=transport&productType=web_player', {
    credentials: 'omit',
    headers: {'User-Agent': 'Mozilla/5.0'}
  });
  if (!res.ok) {
    // Fallback: use client credentials via worker if open endpoint fails
    const clientId = localStorage.getItem(SP_CLIENT_STORE)||'';
    const clientSecret = localStorage.getItem(SP_SECRET_STORE)||'';
    if (!clientId || !clientSecret) throw new Error('Could not get Spotify token. Add Client ID & Secret in ⚙ Settings.');
    const r2 = await fetch(SPOTIFY_PROXY + '/cc-token', {
      method: 'POST',
      headers: {'Content-Type':'application/json','X-Client-Id':clientId,'X-Client-Secret':clientSecret},
      body: '{}',
    });
    if (!r2.ok) throw new Error('Token request failed: ' + r2.status);
    const d2 = await r2.json();
    if (d2.error) throw new Error('Spotify: ' + (d2.error_description||d2.error));
    sessionStorage.setItem('sp_cc_token', d2.access_token);
    sessionStorage.setItem('sp_cc_expiry', String(Date.now()+(d2.expires_in-60)*1000));
    return d2.access_token;
  }
  const data = await res.json();
  const token = data.accessToken;
  if (!token) throw new Error('No token from Spotify');
  const exp = data.accessTokenExpirationTimestampMs || (Date.now() + 3600000);
  sessionStorage.setItem('sp_cc_token', token);
  sessionStorage.setItem('sp_cc_expiry', String(exp - 60000));
  return token;
};
...
const fetchSpotifyPlaylist = async (url) => {
  ...
  // Single worker call — worker handles auth + fetches everything server-side
  ...
  if (!res.ok || data.error) {
    ...
    if (res.status===401||res.status===403)
      throw new Error('Access denied (' + res.status + ') — make sure the playlist is Public in Spotify and your Client ID & Secret are correct');
