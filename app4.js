...
const fetchSpotifyPlaylist = async (url) => {
  ...
  if (!res.ok) {
    ...
    const msg = e.error?.message || ('Status '+res.status);
    if (res.status===401||res.status===403) throw new Error(msg+' — make sure playlist is Public and Client ID+Secret are correct in Settings');
    ...
    h('div',{className:'settings-box'},
      ...
      h('div',{className:'settings-section'},
        ...
        h('input',{className:'inp',style:{marginBottom:'8px'},placeholder:'Paste Client ID',
          ...
          onBlur:e=>{state.spClientId=e.target.value;localStorage.setItem(SP_CLIENT_STORE,e.target.value.trim());}}),
        ...
        h('div',{className:'api-inp-wrap'},
          h('input',{className:'inp',type:'password',placeholder:'Paste Client Secret',
            defaultValue:state.spClientSecret||'',
            onBlur:e=>{state.spClientSecret=e.target.value;localStorage.setItem(SP_SECRET_STORE,e.target.value.trim());}}),
          h('button',{className:'btn btn-green',onClick:()=>{
            sessionStorage.removeItem('sp_cc_token');
            sessionStorage.removeItem('sp_cc_expiry');
            setState({showSettings:false,spotifyStatus:{ok:true,msg:'Spotify credentials saved — paste a playlist URL and tap Import'}});
          }},'Save')
        ),
